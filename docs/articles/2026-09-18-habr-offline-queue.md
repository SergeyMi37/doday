...
А теперь про то, как этот же тест меня обманул.

Внешние адреса из тестового окружения недоступны, поэтому htmx и Alpine я подменяю локальными копиями, а Tailwind — пустой заглушкой. Стили же не влияют на логику очереди, верно?

Значок «ждёт сети» у меня скрывается обычным атрибутом:

```html
<div id="outbox-badge" hidden class="fixed bottom-4 ... flex items-center gap-2">
```

Тест проверял `isVisible()` и честно докладывал: до клика значка не видно, после — видно. Зелёный. А на проде значок висел внизу экрана всегда, у всех, на каждой странице. Написал мне об этом владелец сайта — то есть я же, но с телефона.

Разгадка в приоритетах CSS. Правило `[hidden] { display: none }` живёт в таблице стилей браузера, а `.flex { display: flex }` — в авторской. Специфичность одинаковая, но авторская таблица приоритетнее, и элемент с `hidden` и классом `flex` спокойно показывается. В тесте Tailwind был заглушкой, класса `.flex` не существовало — и атрибут работал как задумано.

```
flex+hidden -> flex | hidden -> none
```

Чинится одной строкой в своих стилях:

```css
[hidden] { display: none !important; }
```

А тест теперь отдаёт заглушку Tailwind с настоящими `display`-утилитами и проверяет не `isVisible()`, а вычисленный `display`. Без исправления он падает:

```
 FAIL  значка не видно, пока очередь пуста — display: flex
```

Вывод, который я себе записал: если подменяешь в тесте внешнюю зависимость пустышкой, ты проверяешь не своё приложение, а его облегчённую версию. Иногда разница — ровно тот баг, который увидят все пользователи.
